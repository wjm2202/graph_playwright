...
import { stitchVideos } from './stitch.mjs';
...
export function ingestFolder(srcDir, { out, openapiPath = null, batchId = null, now = '' , log = () => {} } = {}) {
  ...
  for (const g of buildFromReport(report).guides) {
    ...
    // several per-persona recordings + a timeline → ONE cut (lib/stitch.mjs);
    // anything short of that (one video, no timeline, ffmpeg missing) → copy the first
    const cut = stitchForGuide(g, roots, dest, log);
    if (cut) { stitched[g.slug] = cut; durations[g.slug] = ffprobeMs(dest); }
    else {
      const video = resolveArtifact(g.videoPath, roots);
      if (video) { copyFileSync(video, dest); durations[g.slug] = ffprobeMs(dest); }
...
/**
 * When a test carries several `video` attachments and a `video-timeline`,
 * cut them into one raw.webm that follows the acting persona. Returns the
 * cut summary for guide.json, or null when the plain copy path applies.
 * Never throws: a stitch failure logs and falls back to the first video.
 */
function stitchForGuide(g, roots, dest, log) {
  const tl = g.videoTimeline;
  if (!tl || !Array.isArray(g.videoPaths) || g.videoPaths.length < 2) return null;
  const files = {};
  for (const v of tl.videos ?? []) {
    const match = g.videoPaths.find((p) => path.basename(p) === v.file);
    const resolved = match ? resolveArtifact(match, roots) : null;
    if (resolved) files[v.id] = resolved;
  }
  if (Object.keys(files).length < 2) return null;
  try {
    const plan = stitchVideos(tl, files, dest);
    if (!plan) return null;
    log(`  ⧉ ${g.slug}  stitched ${plan.clips.length} clip(s) from ${Object.keys(files).length} recording(s)${plan.skipped.length ? `  (${plan.skipped.length} interval(s) had no footage)` : ''}`);
    return {
      recordings: (tl.videos ?? []).filter((v) => files[v.id]).map((v) => ({ id: v.id, persona: v.persona, file: v.file, durationMs: plan.durations[v.id] })),
      clips: plan.clips, skipped: plan.skipped,
    };
  } catch (e) {
    log(`  ⚠ ${g.slug}  stitch failed (${(e && e.message) || e}) — using the first recording`);
    return null;
  }
}
