...
| *(new)* `/__library` GET | — | the whole library read off disk per call — `{version, projects:[{name, graphs:[{ref, id, title, tags, sessions, captured, file, doc, invalid?}]}], legacy:[…]}`, each graph validated by the shared schema, its DOCUMENT included (`doc` — the build no longer inlines project graphs, customer material, into the committed planner.html; the served page takes every document from here); an invalid one is listed WITH its errors, never hidden | new | ✓ (served) | ✓ (+ a `suites` key) |
