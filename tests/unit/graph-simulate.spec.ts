...
const GRAPH_FILE = fixturePath('request_to_fulfilment');
...
test('paints every session captured and every oracle pass', () => {
  ...
  for (const s of sessions) {
    ...
    expect(s.steps?.journeyId).toBe('request_to_fulfilment');
  }
...
test('oracle placement follows the after-filters of the walk', () => {
  ...
  const approve = byName.get('request.fulfil');
  expect(approve?.oracles?.map((o) => o.id).sort()).toEqual(['fulfilment_toast', 'order_created']);

  const check = byName.get('siebel.check_order');
  expect(check?.oracles?.map((o) => o.id)).toEqual(['order_visible_in_ui']);

  const assertStep = byName.get('assert.chk_order');
  expect(assertStep?.oracles?.map((o) => o.id).sort()).toEqual(['endpoint_traffic', 'order_in_siebel']);
...
test('a provided screenshot becomes an evidence FILE under sim_<runId>', ({}, testInfo) => {
  ...
  const graphFile = path.join(root, 'graphs', 'request_to_fulfilment.graph.json');
  ...
  const creator = result.graph.nodes.find((n) => n.id === 'sess_sf_requester');
  ...
  expect(creator?.snapshot?.ref).toBe('evidence/request_to_fulfilment/sim_test/sess_sf_requester.jpg');
...
test('a REAL run retires simulated paint on checks it must skip — real prior results survive', () => {
  ...
  // (INVALID_TYPE / no log adapter) — index 5 = e12 (assert.chk_order).
  const skippedReport = {
    journey: 'request_to_fulfilment',
    flags: [],
    steps: [{
      index: 5, kind: 'do' as const, actorAlias: 'siebel_admin', personaId: 'siebel_admin',
      name: 'assert.chk_order', ms: 1200, status: 'ok' as const,
      oracles: [
        { id: 'order_in_siebel', kind: 'api.record_exists', status: 'skipped' as const, message: 'INVALID_TYPE — lives in Siebel' },
        { id: 'endpoint_traffic', kind: 'log.traffic', status: 'skipped' as const, message: 'no adapter bound' },
      ],
    }],
  };
  const mergeOpts = { journeyId: 'request_to_fulfilment', stepEdgeIds: walk.stepEdgeIds, runId: 'real_1' };

  const real = mergeRunIntoGraph(sim.graph, skippedReport, mergeOpts);
  const chk = real.graph.nodes.find((n) => n.id === 'chk_order');
  expect(chk?.expects?.find((x) => x.id === 'order_in_siebel')?.lastResult).toBeUndefined();
  ...
  const prior = painted.nodes.find((n) => n.id === 'chk_order')!.expects!.find((x) => x.id === 'order_in_siebel')!;
  ...
  expect(kept.graph.nodes.find((n) => n.id === 'chk_order')!.expects!.find((x) => x.id === 'order_in_siebel')!.lastResult)
  ...
  expect(simAgain.graph.nodes.find((n) => n.id === 'chk_order')!.expects!.find((x) => x.id === 'order_in_siebel')!.lastResult)
...
test('generatedStepsModule lists the vocabulary and is marked simulated', () => {
  ...
  expect(src).toContain('export function registerSteps_request_to_fulfilment(');
  for (const name of [
    'request.create', 'request.triage', 'risk.check',
    'request.fulfil', 'siebel.check_order',
  ]) {
